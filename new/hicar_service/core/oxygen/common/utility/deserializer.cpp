...
deserializer::deserializer(std::size_t _buffer_shrink_threshold)
...
deserializer::deserializer(const byte_t * _data, std::size_t _length,
                           std::size_t _buffer_shrink_threshold)
...
bool deserializer::deserialize(uint16_t& _value) {
    ...
    _value = OXYGEN_BYTES_TO_WORD(byte0, byte1);
...
bool deserializer::look_ahead(std::size_t _index, uint16_t &_value) const {
    ...
    _value = OXYGEN_BYTES_TO_WORD(*i, *(i+1));
...
bool deserializer::look_ahead(std::size_t _index, uint32_t &_value) const {
    ...
    _value = OXYGEN_BYTES_TO_LONG(*i, *(i+1), *(i+2), *(i+3));
